...
class TestLayers(unittest.TestCase):
    ...
    def test_conv_words(self):
        ...
        def conv_words_impl(input_data, weights, bias, window_size, num_filters):
            ...
            for filter_ind in range(weights.shape[-1]):
                for i in range(batch_size):
                    for j in range(sentence_length - window_size + 1):
                        ...
                        result[i,j,0,filter_ind] = \
                                np.sum(window * filter_weights) + bias[filter_ind]
            return result
        ...
        with tf.variable_scope(name, reuse=True):
            ...
            bias_var = tf.get_variable("b")
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            weights, bias = sess.run([weights_var, bias_var])
            result = sess.run(conv)
        result_comp = conv_words_impl(example, weights, bias,
